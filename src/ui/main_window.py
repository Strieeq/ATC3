...
Адаптировано под модульную структуру проекта.
"""
import sys
import secrets
import string
...
from src.ui.styles import STYLE, make_sep, make_label, C_LILAC, C_ORANGE, C_TEXT2, C_TEXT3, C_GREEN, C_RED, C_BG2, C_BG3
from src.worker import CryptoWorker
from src.crypto import AtomicCipherV3, HardwareFingerprint, ARGON2_OK, CRYPTO_OK


class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Atomic TriFlow Cipher  v3.0")
    ...
    def _init_ui(self):
        ...
        # Header
        hdr = QHBoxLayout()
        lock_lbl = make_label("🔐", size=28)
        hdr.addWidget(lock_lbl)
        title = make_label("Atomic TriFlow Cipher", C_LILAC, 20, bold=True)
        hdr.addWidget(title)
        ver = make_label("v3.0", C_ORANGE, 13)
        hdr.addWidget(ver)
        hdr.addStretch()

        # Badges        for lib, ok, label in [
    ...
    def _tab_encrypt(self):
        ...
        b.setFixedWidth(100)
        b.clicked.connect(lambda: self._browse(self.enc_file, self.enc_text))
        fr.addWidget(b)
        gl.addLayout(fr)
        l.addWidget(grp_in)
        ...
        br.addWidget(cp_btn)
        sv_btn = QPushButton("💾 Сохранить")        sv_btn.setObjectName("btn_secondary")
        sv_btn.clicked.connect(lambda: self._save_text(self.enc_result, "encrypted.atc3"))
        br.addWidget(sv_btn)
        l.addLayout(br)
    ...
    def _tab_decrypt(self):
        ...
        grp_in = QGroupBox("ЗАШИФРОВАННЫЕ ДАННЫЕ (BASE64)")
        gl = QVBoxLayout(grp_in)
        self.dec_data = QTextEdit()
        self.dec_data.setPlaceholderText("Вставь зашифрованный блок (base64)...")
        self.dec_data.setMinimumHeight(100)
        gl.addWidget(self.dec_data)

        fr = QHBoxLayout()
        self.dec_file = QLineEdit()
        self.dec_file.setPlaceholderText("Или выбери файл .atc3...")
        fr.addWidget(self.dec_file)
        b = QPushButton("📂 Обзор")
        b.setObjectName("btn_secondary")
        b.setFixedWidth(100)
        b.clicked.connect(lambda: self._browse_text(self.dec_file, self.dec_data))
        fr.addWidget(b)
        gl.addLayout(fr)
        l.addWidget(grp_in)
        ...
        show_btn2.setFixedWidth(40)        show_btn2.setCheckable(True)
        ...
        sv_btn.clicked.connect(lambda: self._save_text(self.dec_result, "decrypted.txt"))
        br.addWidget(sv_btn)
        l.addLayout(br)
    ...
    def _tab_keygen(self):
        ...
        self.key_out.setPlaceholderText("Сгенерированный ключ появится здесь...")        gl.addWidget(self.key_out)
    ...
    def _tab_info(self):
        ...
        256-битный ключ, 96-битный nonce (случайный). Poly1305 тег = 128 бит.<br>
        ...
        <p><b>4. HMAC-BLAKE2b</b> — финальная подпись всего пакета.<br>        32 байта. Проверяется до расшифровки — защита от oracle атак.</p>
        ...
        <p style='color:{C_TEXT3}; font-size:11px; margin-top:12px'>
        ATC v3 — симметричная схема. Для обмена ключами между двумя людьми
        используй отдельный защищённый канал или реализуй X25519 DH обмен.
        </p>
        """)
        l.addWidget(info)
        return w
    ...
    def _do_encrypt(self):
        ...
        self._run_worker(
            CryptoWorker('encrypt', data, key, self.enc_hw.isChecked()),
            self._on_encrypt_done
        )
    def _do_decrypt(self):
        data = self.dec_data.toPlainText().strip()
        key  = self.dec_key.text()
        file_path = self.dec_file.text()

        if not data and not file_path:
            QMessageBox.warning(self, "Ошибка", "Вставь данные или выбери файл")
            return
        if len(key) < 16:
            QMessageBox.warning(self, "Ошибка", "Ключ минимум 16 символов")
            return

        if not data and file_path and Path(file_path).exists():
            data = Path(file_path).read_text(encoding='utf-8').strip()

        self._run_worker(
            CryptoWorker('decrypt', data, key, False),
    ...
    def _on_decrypt_done(self, ok, err, data):
        if ok:
            try:
                self.dec_result.setPlainText(data.decode('utf-8'))
                self.status_bar.showMessage(f"Расшифровано ✓  ({len(data)} байт)")
            except:
                self.dec_result.setPlainText(f"[бинарные данные, {len(data)} байт]\n" + data.hex())
                self.status_bar.showMessage("Расшифровано (бинарные данные)")
        else:
            QMessageBox.critical(self, "Ошибка дешифрования", err)
            self.status_bar.showMessage("Ошибка")

    def _gen_key(self, length):
        chars = string.ascii_letters + string.digits + "!@#$%^&*-_=+[]{}|"        key = ''.join(secrets.choice(chars) for _ in range(length))
    ...
    def _browse(self, path_edit, text_edit):
        ...
        if path:
            ...
            try:
                text_edit.setPlainText(Path(path).read_text(encoding='utf-8', errors='replace'))
            except:
                text_edit.setPlainText("(бинарный файл — будет зашифрован)")

    def _browse_text(self, path_edit, text_edit):
        path, _ = QFileDialog.getOpenFileName(self, "Открыть зашифрованный файл", "", "*.atc3 *.txt *.bin;;Все файлы (*)")
        if path:
            path_edit.setText(path)
            try:
                text_edit.setPlainText(Path(path).read_text(encoding='utf-8').strip())
            except:
                pass
    ...
    def _copy(self, widget):
        ...
        if text:
            self.clipboard = self.clipboard() if hasattr(self, 'clipboard') else QApplication.clipboard()
            from PyQt6.QtWidgets import QApplication
            QApplication.clipboard().setText(text)
            self.status_bar.showMessage("Скопировано в буфер")

    def _save_text(self, widget, default_name):
        text = widget.toPlainText()
        if not text:
            return
